...
#   TG_PROVIDER_CACHE=1 terragrunt run --non-interactive --working-dir backend-tfstate-bootstrap/dev -- apply -auto-approve --parallelism 50 
#   TG_PROVIDER_CACHE=1 terragrunt run --non-interactive --working-dir backend-tfstate-bootstrap/prod -- apply -auto-approve --parallelism 50 
#   exit 0
# fi
...
# BACKEND_DEV_STATE=$(find backend-tfstate-bootstrap/dev/.terragrunt-cache -type f -name "terraform.tfstate" 2>/dev/null | head -n 1)
# BACKEND_PROD_STATE=$(find backend-tfstate-bootstrap/prod/.terragrunt-cache -type f -name "terraform.tfstate" 2>/dev/null | head -n 1)
# echo "$BACKEND_STATE"
