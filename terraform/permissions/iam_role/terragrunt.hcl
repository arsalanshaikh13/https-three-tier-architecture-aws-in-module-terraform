...
terraform {
  # source = "../../../../modules/app"
  # source = "${path_relative_from_include("root")}/modules/permissions/iam_role"
  source = "tfr://gitlab.com/arsalanshaikh13/tf-modules-panda-user-data/aws//permissions/iam_role?version=1.0.0-data-route53"
  # Notice the git:: prefix and the https protocol
  # source = "git::https://gitlab.com/arsalanshaikh13/tf-modules-panda-user-data.git//modules/permissions/iam_role?ref=main"
  # source = "git::ssh://git@gitlab.com/arsalanshaikh13/tf-modules-panda-user-data.git//modules/permissions/iam_role?ref=main"
  # source = "git::https://github.com/arsalanshaikh13/https-three-tier-architecture-aws-in-module-terraform.git//modules/permissions/iam_role?ref=v1-terragrunt"
  # source = "git::ssh://git@github.com/arsalanshaikh13/https-three-tier-architecture-aws-in-module-terraform.git//modules/permissions/iam_role?ref=v1-terragrunt"
  ...
  before_hook "test_terragrunt" {
    commands = ["plan"]
    execute  = ["bash", "-c", "echo '${get_original_terragrunt_dir()}'/custom.tflint.hcl; echo '${get_terragrunt_dir()}'/custom.tflint.hcl; echo '${get_parent_terragrunt_dir("root")}'/custom.tflint.hcl; "]
  }
  #  before_hook "test_terragrunt2" {
  #   commands = ["plan"]
  #   execute  = ["echo ${get_original_terragrunt_dir()}/custom.tflint.hcl; echo ${get_terragrunt_dir()}/custom.tflint.hcl; echo ${get_parent_terragrunt_dir("root")}/custom.tflint.hcl; "]
  # }
